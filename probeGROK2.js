// GROK-002：把 1×1 馬廄精靈存成白天圖，並在種子城找一棟 v 對上 mews 的住宅拍近景。
'use strict';
const fs = require('fs');
const path = require('path');
const { withGame, sleep, ROOT } = require('./harness.js');
(async () => {
  const r = await withGame({ port: 8293, timeout: 400, log: () => {} }, async ({ cdp }) => {
  ...
  const dir = path.join(ROOT, 'shotsGROK2');
