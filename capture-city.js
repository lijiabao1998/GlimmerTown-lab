...
(async () => {
  ...
  const session = await withGame({ port: 8199, timeout: 300, log }, async ({ cdp }) => {
    ...
    const ver = await cdp.evalJs(`(window.GV && (window.GV.build546||window.GV.build545||window.GV.build544)) ? (window.GV.build546||window.GV.build545||window.GV.build544)() : {ver:'?',anchor:'?'}`);
