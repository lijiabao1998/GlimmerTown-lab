...
(async () => {
  ...
  const session = await withGame({ port: 8199, timeout: 300, log }, async ({ cdp }) => {
    ...
    const ver = await cdp.evalJs(`(window.GV && (window.GV.build552||window.GV.build548||window.GV.build547||window.GV.build546)) ? (window.GV.build552||window.GV.build548||window.GV.build547||window.GV.build546)() : {ver:'?',anchor:'?'}`);
