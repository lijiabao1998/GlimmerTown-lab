...
const FAM=arg('fam','deco'),S=+arg('scale',3),OUT=arg('out','shots575/fam.png'),PORT=+arg('port',8199),SNIP=arg('snippet','');
if(PORT===8123){console.error('埠 8123 禁用');process.exit(2);}
const preScript=SNIP?fs.readFileSync(SNIP,'utf8'):'';
(async()=>{const r=await withGame({port:PORT,timeout:300,enterCity:false,preScript,log:()=>{}},async({cdp})=>{
  ...
  return await ev(`(()=>{const SP=GV.art574.SPR();const L=[];const add=(k,o)=>{const im=o&&o.img?o.img:(o&&o.getContext?o:null);if(im)L.push([k,im,o&&o.night?o.night:null]);};
    ...
    const S=${S},cw=Math.max(...L.map(q=>q[1].width))*S+8,ch=Math.max(...L.map(q=>q[1].height))*S+20,C=Math.min(6,L.length),R=Math.ceil(L.length/C)*2;
    ...
    L.forEach(([k,im,nm],i)=>{const X=(i%C)*cw,Y=Math.floor(i/C)*ch*2;g.strokeStyle='#355';g.strokeRect(X,Y,cw,ch);g.drawImage(im,X+4,Y+16,im.width*S,im.height*S);g.fillStyle='#141a2c';g.fillRect(X,Y+ch,cw,ch);g.save();g.filter='brightness(0.36) saturate(0.7)';g.drawImage(im,X+4,Y+ch+16,im.width*S,im.height*S);g.restore();if(nm)g.drawImage(nm,X+4,Y+ch+16,im.width*S,im.height*S);g.fillStyle='#fff';g.font='bold 13px sans-serif';g.fillText(k+' '+im.width+'x'+im.height,X+3,Y+13);});
